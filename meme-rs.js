...
export class memes extends plugin {
  ...
  async init () {
    ...
    if (Object.keys(infos).length === 0 || Object.keys(keyMap).length === 0) {
      // 只能本地生成了
      let infoRes = await fetch(`${baseUrl}/meme/infos`)
      let info = await infoRes.json()

      let keyMapTmp = {}
      let infosTmp = {}
      for (const memeInfo of info) {
        let key = memeInfo.key
  ...
  async memesUpdate (e) {
    ...
    await e.reply('更新完成')
    const resultFileLoc = 'data/memes/render_list1.jpg'
  ...
  async memesList (e) {
    ...
    try {
      ...
      let response = await fetch(baseUrl + '/tools/render_list', {
        method: 'POST',
        headers: {
  ...
  async memes (e) {
    ...
    let textList = text.split('/', info.params.max_texts)
    ...
    if (info.params.max_texts > 0 && texts.length === 0) {
      if (texts.length < info.params.max_texts) {
        if (e.message.filter(m => m.type === 'at').length > 0) {
          texts.push(_.trim(e.message.filter(m => m.type === 'at')[0].text, '@'))
        } else {
    ...
    console.log('input', { target, targetCode, images: imageIds, texts, options })
